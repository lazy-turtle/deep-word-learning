...
def show_compactness(xs, ys, num_classes, seed):
    print('Fitting clustering model...')
    model = KMeans(num_classes, max_iter=10000, tol=1e-10, random_state=seed)
...
def show_clustering(xs, ys, num_classes, labels, name, show_classes=False, iter=1000, tol=1e-6, seed=42):
    print('Fitting clustering model...')
    model = KMeans(num_classes, max_iter=iter, tol=tol, random_state=seed)
...
audio_data_names = [
    'audio-10classes-20pca25t.csv',
    'audio-10classes-coco-imagenet_train.csv',
    ...
    if not args.classes100:
        ...
        if not args.is_audio:
            ...
            if args.data == 'new':
                ...
                labels = utils.labels_dictionary(labels_path)
            else:
